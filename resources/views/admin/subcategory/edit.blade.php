                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="flex-wrap d-flex justify-content-between align-items-center">
                                        ...
                                        <div>
                                            <a href="{{url('subcategory')}}" class="btn btn-link btn-soft-light">
...
<div class="row">
   <div class="col-sm-12">
      <div class="card">
         ...
         <div class="card-body">
        <form action="{{url('subcategory/subcatupdate/'.$list->id)}}"  method="post" enctype="multipart/form-data">
            @csrf
         <div class="row">
         <div class="col-md-6 mt-3" >
                    <label class="form-label" for="price">Super Category</label>
                    <select name="sid" id="from" class="form-control" required>
                        @foreach($catlist as $row)
                        <option @if($list->sid== $row->id) selected @endif value="{{$row->id}},{{$row->cat_name}}">{{$row->cat_name}}</option>
                        @endforeach
                    </select>
                    @if($errors->has('sid'))
                        <div class="form-error">{{ $errors->first('sid') }}</div>
                    @endif
                </div>
                <div class="col-md-6 mt-3" >
                    <label class="form-label" for="price">Category</label>
                    <select name="cid" id="cat-id" class="form-control" required>
                        <option value="{{$list->cid}},{{$list->cname}}">{{$list->cname}}</option>
                    </select>
                    @if($errors->has('cid'))
                        <div class="form-error">{{ $errors->first('cid') }}</div>
                    @endif
                </div>

                <div class="col-md-6 mt-3" >
                    <label class="form-label" for="price">Sub Category</label>
                    <input type="text" class="form-control" name="scname" value="{{$list->scname}}"  placeholder="Category Name" required>
                    @if($errors->has('scname'))
                        <div class="form-error">{{ $errors->first('scname') }}</div>
                    @endif
                </div>
