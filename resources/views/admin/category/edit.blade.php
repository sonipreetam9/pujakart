            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="flex-wrap d-flex justify-content-between align-items-center">
                        ...
                        <div>
                            <a href="{{url('category')}}" class="btn btn-link btn-soft-light">
...
<div class="row">
   <div class="col-sm-12">
      <div class="card">
        ...
        <div class="card-body">
        <form action="{{url('category/catupdate/'.$list[0]->sid)}}"  method="post" enctype="multipart/form-data">
            @csrf
            <div class="row">
                ...
                <div class="col-md-6 mt-3" >
                    ...
                    <input type="text" class="form-control" name="cname" value="{{$list[0]->cname}}" placeholder="Category Name" required>
                    @if($errors->has('cname'))
                        <div class="form-error">{{ $errors->first('cname') }}</div>
                    @endif
                </div>
                <div class="col-md-6 mt-3" >
                    <label class="form-label" for="price">Parent Category</label>
                    <select name="is_parent" class="form-control" required>
                        <option value="0">--Select--</option>
                        @foreach($cat as $row)
                            @if($list[0]->cname != $row->cname)
                                <option @if($list[0]->is_parent == $row->sid) selected @endif value="{{$row->sid}}">{{$row->cname}}</option>
                            @endif
                        @endforeach
                    </select>
                </div>
                <div class="col-md-6 mt-3">
                    <label class="form-label" for="header">Display In Header</label>
                    <select name="head" class="form-control">
                        <option @if($list[0]->head==0) selected @endif value="0">-- Select --</option>
                        <option @if($list[0]->head==1) selected @endif value="1">Head</option>
                    </select>
                </div>
                <div class="col-md-6 mt-3" >
