...
<div class="row">
   <div class="col-sm-12">
      <div class="card">
         ...
         <div class="card-body">
            <form action="{{url('category/cat_order')}}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="row">
            ...
            <!--   <table id="datatable" class="table " data-toggle="data-table">-->
                ...
                <table id="table" class="draggable-table tblContainer">
                  ...
                  <tbody>
                    ...
                    <tr id="{{$item->sid}}">
                        <input type="hidden" name="p[]" value="{{$item->sid}}">
                        <td>#{{$i}}</td>
                      
                        <td><img src="{{asset('categoryImage/'.$item->image)}}" alt="" style="width:50px;"></td>
                        <td>{{$item->cname}}</td>
                         <td>
                            @if($item->status==1) 
                                <a class="mt-2 badge bg-success" onclick="test({{$item->sid}})">Unhide</a> 
                            @else 
                                <a class="mt-2 badge bg-danger" onclick="test({{$item->sid}})">Hide</a>  
                            @endif
                        </td>
                        <td><?php if($item->is_parent==0){ echo "<span class='badge bg-success'>Category</span>";}
                            else{echo "<span class='badge bg-danger'>Sub Category</span>";} ?>
                        </td>
                        ...
                        <td>{{$i}}</td>
                        <td>
                            <a href="{{url('category/edit/'.$item->sid)}}"> 
                            ...
                            <a href="{{url('category/delete/'.$item->sid)}}" onclick="return confirm('Are you sure to Delete Permanently?')"> 
...
<script>
    ...
    var ur = "https://shripujansamagri.com/category/statushide/";
