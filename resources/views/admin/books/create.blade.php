    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{url('books/booksave')}}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <div class="col-md-6 mt-3">
                                <label class="form-label" for="cname">Book Name<span style="color:red;">*</span></label>
                                <input type="text" class="form-control" name="book_name" placeholder="Book Name" required>
                                @if($errors->has('book_name'))
                                    <div class="form-error">{{ $errors->first('book_name') }}</div>
                                @endif
                            </div>
                            <div class="col-md-6 mt-3">
                                <label class="form-label" for="header">Category<span style="color:red;">*</span></label>
                                <select name="bcname" class="form-control" required>
                                    <option value="">-- Select --</option>
                                    @foreach($list as $row)
                                    <option value="{{$row->id}},{{$row->bookname}}">{{$row->bookname}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6 mt-3">
                            ...
                            <div class="col-md-6 mt-3">
                                ...
                                <input type="file" name="bookpdf" accept="application/pdf" class="form-control" required>
                            </div>
                            <!--<div class="col-md-12 mt-3" >
                                <label class="form-label" for="price">Full Description</label>
                                <textarea name="book_detail" class="ckeditor form-control"></textarea>
